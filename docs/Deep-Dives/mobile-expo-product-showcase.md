...
- Remove `FloatButton` component, as we don't want to allow users to add the products
  ```
  <FloatButton
...
- Comment the swipeable behaviour on the list, to disable edit and delete button when swipe item
  ```
  // isSwipeable
  ```
...
In the `ObjectScreen.js` file we want to remo additional menu options (edit, delete).
```
// headerRight: () => (
//     <MenuDropdown
//         items={items}
...
Here's the [full list of changes](https://github.com/netboxsoft/flotiq-mobile-demo/pull/1/files) that have to be made to simplify the original app, in case you missed something.
